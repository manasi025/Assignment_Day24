...
public class Person implements AddressBookIF 
{
		...
		while (numOfContact > 0)
		{
			...
			if (!detailsBook.containsKey(firstName))
			{
				...
				detailsBook.put(firstName,
						new Contacts(firstName, lastName, address, city, state, zip, phoneNumber, email));
				numOfContact--;
			} 
			else
			{
	...
	public void searchPerson() 
	{
		...
		String name = sc.next();
		Map<String, Contacts> searchedPerson = detailsBook.entrySet().stream().filter(e -> e.getKey().equals(name))
	...
	public void viewByCity() 
	{
		...
		String cityName = sc.next();
		Map<String, Contacts> detailsByCity = personByCity.entrySet().stream().filter(e -> e.getKey().equals(cityName))
				.collect(Collectors.toMap(e -> e.getKey(), e -> e.getValue()));
	...
	public void viewByState() 
	{
		...
		String stateName = sc.next();
		Map<String, Contacts> detailsByState = personByState.entrySet().stream()
	...
	public void sortAddressBook()
	{
		...
		Map<String, Contacts> sortedContact = detailsBook.entrySet().stream().sorted(Entry.comparingByKey()).collect(
	...
	public void sortByCity() 
	{
		...
		Map<String, Contacts> sortedByCity = personByCity.entrySet().stream().sorted(Entry.comparingByKey()).collect(
				Collectors.toMap(Entry::getKey, Entry::getValue, (oldValue, newValue) -> oldValue, LinkedHashMap::new));
	...
	public void sortByState() 
	{
		...
		Map<String, Contacts> sortedByState = personByState.entrySet().stream().sorted(Entry.comparingByKey()).collect(
				Collectors.toMap(Entry::getKey, Entry::getValue, (oldValue, newValue) -> oldValue, LinkedHashMap::new));
